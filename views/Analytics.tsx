...
import { toDataSources, getSourceById } from '../utils/dataSources';
...
const Analytics: React.FC<AnalyticsProps> = ({ project, onUpdateProject }) => {
  ...
  const availableColumns = useMemo(() => {
      ...
      return defaultSource.columns;
  }, [defaultSource]);

  const applyGlobalFilters = (rows: RawRow[]) => {
  ...
  const filteredData = useMemo(() => applyGlobalFilters(defaultSource?.rows || []), [defaultSource, filters]);
  ...
  const getDataForWidget = (widget: DashboardWidget) => {
      const sourceData = getSourceById(project, widget.dataSourceId)?.rows || [];
      const scoped = applyGlobalFilters(sourceData);
      return applyWidgetFilters(scoped, widget.filters);
  };
  ...
  const handleSaveWidget = async (newWidget: DashboardWidget) => {
      const widgetWithSource = { ...newWidget, dataSourceId: newWidget.dataSourceId || defaultSource?.id };
      let updatedWidgets = [...widgets];
      if (editingWidget) {
          updatedWidgets = updatedWidgets.map(w => w.id === widgetWithSource.id ? widgetWithSource : w);
      } else {
          updatedWidgets.push(widgetWithSource);
      }
  ...
  const handleChartClick = (e: any, widget: DashboardWidget, activeLabel?: string) => {
      ...
      const sourceData = getSourceById(project, widget.dataSourceId)?.rows || [];
      const scopedData = applyWidgetFilters(applyGlobalFilters(sourceData), widget.filters);
      ...
      if (interactionMode === 'filter') {
          ...
          // Drill Down Logic
          const clickedData = scopedData.filter(row => {
  ...
  const processWidgetData = (widget: DashboardWidget) => {
      const widgetData = getDataForWidget(widget);
      // 1. TABLE WIDGET
