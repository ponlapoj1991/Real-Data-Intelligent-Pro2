...
const ChartBuilder: React.FC<ChartBuilderProps> = ({
  ...
  const [selectedSourceId, setSelectedSourceId] = useState<string | undefined>(initialWidget?.dataSourceId || defaultSourceId || dataSources[0]?.id);

  const activeSource = useMemo(() => dataSources.find((s) => s.id === selectedSourceId) || dataSources[0], [dataSources, selectedSourceId]);
  ...
  const detectSmartDefaults = () => {
      ...
      for (const col of activeColumns) {
          ...
          const isNumeric = activeRows.slice(0, 20).every(r => {
              const val = r[col];
  ...
  useEffect(() => {
    if (isOpen) {
        if (initialWidget) {
            ...
            setSelectedSourceId(initialWidget.dataSourceId || defaultSourceId || dataSources[0]?.id);
        } else {
            ...
            setShowGrid(true);
            setFilters([]);
            setSelectedSourceId(defaultSourceId || dataSources[0]?.id);
            detectSmartDefaults();
        }
    }
  }, [isOpen, initialWidget, defaultSourceId, dataSources]);
        ...
        <div className="flex-1 flex overflow-hidden">
            ...
            <div className="w-1/3 border-r border-gray-200 bg-gray-50/50 flex flex-col overflow-y-auto p-6 space-y-6 custom-scrollbar">
                ...
                <div className="space-y-4 bg-white p-4 rounded-xl border border-gray-200 shadow-sm">
                    <div>
                        <label className="block text-xs font-bold text-gray-700 mb-1">Data Source</label>
                        <select
                        ...
                        >
                            {dataSources.map((ds) => (
                                <option key={ds.id} value={ds.id}>
                                    {ds.kind === 'raw' ? 'Raw: ' : 'Prepped: '}{ds.label}
                                </option>
                            ))}
                        </select>
                    </div>
